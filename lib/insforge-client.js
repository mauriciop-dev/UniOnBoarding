...
export async function storeFeedback({ url, platform, provider, lang, rating, comment }) {
  if (!isReady()) return { stored: false, reason: 'not_configured' };

  try {
    await request('/api/database/records/feedback', {
      method: 'POST',
      headers: {
    ...
    console.warn('[InsForge] feedback store error:', err.message);
