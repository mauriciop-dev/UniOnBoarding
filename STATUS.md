...
- Manifest `0.1.15` (`voice-worklet.js` web-accessible; mic vía **offscreen document** `USER_MEDIA` + permiso pedido desde `request-mic.html`, sin `audioCapture`).
...
- **Micrófono**: Chrome no muestra el prompt en el sidepanel (bug conocido). Captura en `offscreen.html` (USER_MEDIA) + `request-mic.html` pide el permiso la primera vez. El PCM16 viaja del offscreen al sidepanel por messaging.
